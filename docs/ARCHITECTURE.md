...
calls `draw()` with the remaining accumulator fraction. Armies retain previous
and current leg progress, and only drawing interpolates between them. Simulation
decisions therefore remain deterministic for a given command sequence.
